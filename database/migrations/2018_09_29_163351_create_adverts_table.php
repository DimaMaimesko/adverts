...
class CreateAdvertsTable extends Migration
{
    public function up()
    {
        Schema::create('advert_adverts', function (Blueprint $table) {
            $table->increments('id');
            $table->integer('user_id')->references('id')->on('users')->onDelete('CASCADE');  //если удаляется пользователь - удаляем все его обьявления
            $table->integer('category_id')->references('id')->on('advert_categories');       //если удаляется категория - запрещаем удалять обьявления к ней привязанные (restrict - по умолчанию)
            $table->integer('region_id')->nullable()->references('id')->on('advert_regions');//если удаляется регион - запрещаем удалять регион к ней привязанный (restrict - по умолчанию)
            $table->string('title');
            $table->integer('price');
            $table->text('address');
            $table->text('content');
